...
# 1. ตั้งค่าหน้าจอแนว Dashboard แบบกว้างเต็มตา
st.set_page_config(page_title="URBAN METROPOLIS ENGINE", page_icon="🏛️", layout="wide")

# 2. ปรับแต่ง CSS ธีมสว่างพรีเมียม ขยายหัวข้อและโลโก้ให้ใหญ่สะใจ ตัวอักษรคมชัดอ่านง่ายที่สุด
st.markdown("""
    <style>
    ...
    .huge-logo { text-align: center; font-size: 85px !important; margin-bottom: 0px; }
    .clean-title { color: #0f172a !important; font-size: 46px !important; font-weight: 900 !important; text-align: center; margin-top: 0px !important; margin-bottom: 5px; letter-spacing: 1px; }
...
# ส่วนหัวเว็บบอร์ดขนาดใหญ่พิเศษ
st.markdown("<div class='huge-logo'>🏛️</div>", unsafe_allow_html=True)
st.markdown("<h1 class='clean-title'>URBAN-NLP : INTELLIGENCE COMMAND CENTER</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #64748b !important; font-size: 16px; font-weight: 600;'>โมเดลวิเคราะห์คำร้องเรียนและบริหารจัดการสารสนเทศเชิงโครงสร้างเมืองอัจฉริยะ (Enterprise Edition)</p>", unsafe_allow_html=True)
...
with col2:
    st.markdown("<p style='font-size: 15px; font-weight: bold; color: #0284c7 !important; margin-bottom: 8px;'>📂 BATCH MODE DATA INPUT</p>", unsafe_allow_html=True)
...
if st.button("🚀 EXECUTE MULTI-STAGE NLP PIPELINE", use_container_width=True):
    if user_input.strip():
        ...
        st.markdown("<br><h3 style='color: #0f172a; font-weight: 700;'>📊 STAGE 1: TELEMETRY METRICS</h3>", unsafe_allow_html=True)
        ...
        with out1:
            st.markdown("<div class='section-box' style='border-top: 4px solid #0284c7;'><b>🔒 MASKED DATA SECURITY</b></div>", unsafe_allow_html=True)
            st.info(cleansed)
        ...
        with out2:
            st.markdown("<div class='section-box' style='border-top: 4px solid #10b981;'><b>🏷️ METADATA EXTRACTION</b></div>", unsafe_allow_html=True)
        ...
        st.markdown("<br><h3 style='color: #0f172a; font-weight: 700;'>⚙️ STAGE 4: URBAN INTELLIGENCE & LOGS</h3>", unsafe_allow_html=True)
        out3, out4 = st.columns(2)
        with out3:
            st.markdown("<div class='section-box' style='border-top: 4px solid #eab308; height: 100%;'><b>💡 STRATEGIC RESPONSE PLAN</b><br><br>ระบบสั่งการอัตโนมัติคัดเลือกแผนกลยุทธ์ที่เหมาะสมกับปัญหาเพื่อส่งต่อให้หน่วยงานส่วนกลาง:</div>", unsafe_allow_html=True)
            st.success(f"📋 **Action Item:** {strategy}")
        with out4:
            st.markdown("<div class='section-box' style='border-top: 4px solid #64748b;'><b>🛠️ SYSTEM EXECUTION SUB-ROUTINES</b></div>", unsafe_allow_html=True)
            log_data = {
